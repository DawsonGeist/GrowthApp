...
def lambda_handler(event, context):
    ...
    body = event['body']
    mood = event['mood']
    sql_string = f'insert into notes (noteID, username, date_time_created, body, mood) values ("{noteID}", "{username}", "{date_time_created}", "{body}", "{mood}")'
    with conn.cursor() as cur:
        cur.execute(sql_string)
        conn.commit()
    conn.commit()
    payload = "Successfully added note"
    # return payload
    return {
        'statusCode': 200,
